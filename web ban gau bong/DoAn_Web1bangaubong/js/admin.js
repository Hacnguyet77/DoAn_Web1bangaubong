...
function addThongKe() {
    ...
    danhSachDonHang.forEach(donHang => {
        ...
        donHang.sp.forEach(sanPhamTrongDonHang => {
            let tenHang = sanPhamTrongDonHang.sanPham.phan_loai;
...
function layThongTinSanPhamTuTable(id) {
    ...
    var phan_loai = tr[3].getElementsByTagName('td')[1].getElementsByTagName('select')[0].value;
    ...
    var quantity = tr[13].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
...
function autoMaSanPham(phan_loai) {
    // hàm tự tạo mã cho sản phẩm mới
    if(!phan_loai) phan_loai = document.getElementsByName('chonphan_loai')[0].value;
    var index = 0;
    for (var i = 0; i < list_products.length; i++) {
        if (list_products[i].phan_loai == phan_loai) {
            index++;
        }
    }
    document.getElementById('maspThem').value = phan_loai.substring(0, 3) + index;
}
...
function addKhungSuaSanPham(masp) {
    ...
    <table class="overlayTable table-outline table-content table-header">
        ...
        <tr>
            ...
            <td>
                ...
                var phan_loai = ["gau bong", "goi","phu kien"];
    for(var c of phan_loai) {
        if(sp.phan_loai == c)
    ...
    s += `
        ...
        <tr>
            <td>quantity:</td>
            <td><input type="text" value="`+sp.detail.quantity+`"></td>
        </tr>
...
// for(var i = 0; i < list_products.length; i++) {
//     list_products[i].masp = list_products[i].phan_loai.substring(0, 3) + vitriphan_loai(list_products[i], i);
// }
